      ...
      <el-main v-loading="detailLoading" class="main-content">
        <el-form label-width="90px" :rules="rules" ref="article" :model="articleForm" id="article-form" size="small">
          <el-form-item label="文章标题:" prop="articleName">
            <el-input type="input" :disabled="true" required v-model="articleForm.articleName"></el-input>
          </el-form-item>
          <el-form-item label="字号" props="wordNum">
            <el-input-number v-model="articleForm.wordNum" :min="0"></el-input-number>
          </el-form-item>
          <el-form-item label="文章内容:">
            <u-editor :config="editorConfig" :type="'edit'" ref="ueditor"></u-editor>
          </el-form-item>
          ...
          <el-form-item label="" class="tags-item" >
            ...
            <span class="no-tags-text" v-if="articleForm.type && articleForm.type.length == 0"> 点击右侧添加</span>
          </el-form-item>
          <el-form-item label="" class="tags-item" >
            <h5 class="form-item-tags-title">
            ...
            <span class="no-tags-text" v-if="articleForm.special && articleForm.special.length == 0"> 点击右侧添加</span>
          </el-form-item>
          <el-form-item >
            <el-row :gutter="20">
              <el-col :span="6">
                <el-checkbox v-model="laudScoreChecked" :disabled="articleOperateType == 'edit' && articleForm.allowLaud">
                  点赞
                  <el-input v-model="articleForm.laudScore" class="score-input" type="number" :disabled="articleOperateType == 'edit' && articleForm.allowLaud"></el-input>
                </el-checkbox>
              </el-col>
              <el-col :span="6">
                <el-checkbox v-model="commentScoreChecked" :disabled="articleOperateType == 'edit' && articleForm.allowComment">
                  评论
                  <el-input v-model="articleForm.commentScore" class="score-input" type="number" :disabled="articleOperateType == 'edit' && articleForm.allowComment"></el-input>
                </el-checkbox>
              </el-col>
              <el-col :span="6">
                <el-checkbox v-model="learnScoreChecked" :disabled="articleOperateType == 'edit' && articleForm.allowLearn">
                  学习
                  <el-input  v-model="articleForm.learnScore" class="score-input" type="number" :disabled="articleOperateType == 'edit' && articleForm.allowLearn"></el-input>
                </el-checkbox>
              </el-col>
              <el-col :span="6">
                <el-checkbox v-model="errorScoreChecked" :disabled="articleOperateType == 'edit' && articleForm.allowError">
                  报错
                  <el-input 
                    v-model="articleForm.errorScore" class="score-input" type="number" :disabled="articleOperateType == 'edit' && articleForm.allowError">
                  </el-input>
                </el-checkbox>
              </el-col>
            </el-row>
  ...
  export default {
    data() {
      return {
        ...
        typeSelectValue:    null,  //添加dialog > 分类的select Value
        ...
        articleOperateType:      'edit', //当前操作模式 id参数为new 新增'new' 其余正常id为编辑'edit'
        articleId:         null,  //文章id
        articleForm: {           //文章表单
    ...
    methods: {
      ...
      getArticleDetail (id) {
        ...
        self.$request.requestAxios('get', `${basePath}/a_info/${id}/detail/from_org/${self.orgCode}`, null, 
          (response) => {
            ...
            if(self.articleForm.privateFiles) {
              self.fileList = JSON.parse(self.articleForm.privateFiles)
            } else {
              self.fileList = []
            }
      ...
      submitArticle () {
        ...
        self.$refs.article.validate((valid) => {
          if (valid) { //验证通过
            ...
            self.$request.requestAxios("put", `${basePath}/a_info/article/from_org/${self.orgCode}`, self.articleForm, 
              (response) => {
