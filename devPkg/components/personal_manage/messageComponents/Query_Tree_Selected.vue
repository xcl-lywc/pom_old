...
<script>
  import Show_Tree_Selected_Last  from '../../task_manage/detail/Show_Tree_Selected_Last.vue'
  export default {
    props:{
    ...
    methods:{
      ...
      getQueryData(paramsData) {   
        ...
        this.$request.requestAxios('get', `${basePath}/${paramsData.requestUrl}`, {params: {id:  this.userData.partyOrgId}},
          (response) => {
      ...
      _convertData(data){ 
        ...
        data.forEach((item) => {
          item.isShow = false;
          if(item.id == this.paramsData.value){
