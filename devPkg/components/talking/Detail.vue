<template>
  <div class="content-manage">
    <el-button type="primary" @click="$router.push({name: 'Talking_Talking'})">返回</el-button>
    <div class="detail-wrap" v-loading="detailLoading">
      ...
      <p>
        ...
        <!-- <el-upload
          ...
          :on-remove="handleTalkingRemove"
          :on-success="handleTalkingSuccess"
          :before-upload="beforeFileUpload"
          :limit="1"
          :file-list="detailData.recordInfo">
            <el-button size="small" type="primary">上传谈话记录表</el-button>
  ...
  export default {
    ...
    methods: {
      ...
      handleRemove (file, filesList) {
        ...
        self.$request.requestAxios("post", `${basePath}/conversation/private_file/update/${self.$route.params.id}/from_org/${self.orgCode}`,
          {
            fileIds: filesList.map((item) => { return item.id }).join(",")
          },
          (response) => {
            self.getDetail()
          },
