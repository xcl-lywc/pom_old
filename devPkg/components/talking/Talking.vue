        ...
        <el-form ref="talkingForm" :model="talkingForm" :rules="rules" label-width="100px" size="small">
          <el-col :span="14">
            <el-form-item label="谈话人" prop="talkingList">
              <el-input type="text" readonly v-model="talkingListName" @focus="activeType = 'talking'"></el-input>
            </el-form-item>
            <el-form-item label="被谈话人" prop="beTalkingList">
              <el-input type="text" readonly v-model="beTalkingListName" @focus="activeType = 'beTalking'"></el-input>
            </el-form-item>
          ...
          <el-col :span="10" class="right-people-list-wrap">
            ...
            <tree-selected 
              ...
              v-show="activeType == 'talking'"
              pageType="org"
              @treeCheckedData="peopleTreeChanged">
            </tree-selected>
            <tree-selected 
              selectionType="checked"
              v-show="activeType == 'beTalking'"
              pageType="org"
              @treeCheckedData="peopleTreeChanged">
            </tree-selected>
            <tree-selected 
              selectionType="radio"
              v-show="activeType == 'talkingRecord'"
              pageType="org"
              @treeCheckedData="peopleTreeChanged">
            </tree-selected>
          </el-col>
  ...
  export default {
    ...
    methods: {
      ...
      getTalkingListData (page, perPage) {
        ...
        self.$request.requestAxios("post", `${basePath}/conversation/conversation/list/from_org/${self.orgCode}`, paramObj, 
          (response) => {
      ...
      submitForm(formName) {
        ...
        self.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            self.$request.requestAxios("post", `${basePath}/conversation/conversation/add/from_org/${self.orgCode}`, submitForm, 
              (response) => {
      ...
      handleFormToService () {
        ...
        //处理人员
        deepClone.initiatores = self.talkingForm.talkingList.map((item) => {
          return item.id
        })
        deepClone.talkers     = self.talkingForm.beTalkingList.map((item) => {
          return item.id
        })
