...
<template>
  <el-form ref="form" :model="form" label-width="130px">
    ...
    <el-form-item 
      ...
      :rules="[{ required: true, message: '请上传政审材料', trigger: ['blur', 'click'] },]">
      <el-upload
          ...
          :on-success="(($event)=>{handleAvatarSuccess($event, 'politicalTrialFile')})"
    ...
    <el-form-item 
      label="培养联系人："
      prop="introducer"
      :rules="[{ required: true, message: '请输入培养联系人', trigger: ['blur', 'click'] },]">
      <el-input v-model="form.introducer" style="max-width: 400px;"></el-input>
    </el-form-item>
    <el-form-item 
      label="考察谈话记录："
      prop="talkRecordFile"
      ...
      <el-upload
          ...
          :on-preview="handlePreview"
          :on-remove="handleRemove"
          :on-success="(($event)=>{handleAvatarSuccess($event, 'talkRecordFile')})"
          :before-upload="beforeAvatarUpload"
          :before-remove="beforeRemove"
          :file-list="fileList"
          :multiple="false"
    ...
    <el-form-item 
      label="支部会议时间：" 
      prop="meetingDate"
      :rules="[{ required: true, message: '请上传支部会议时间', trigger: ['blur', 'change'] },]">
      <el-date-picker
    ...
    <el-form-item 
      label="投票记录表：" 
      prop="voteRecordFile"
      :rules="[{ required: true, message: '请上传投票记录表', trigger: ['blur', 'click'] },]">
      <el-upload
          ref="voteRecordFile"
          class="upload-demo upload-demo-manage"
          :action="`${$util.basePath}/file/${orgId}/uplode`"
    ...
    <el-form-item 
      ...
      :rules="[{ required: true, message: '请上传入党志愿书', trigger: ['blur', 'click'] },]">
      <el-upload
          ...
          :on-preview="handlePreview"
          :on-remove="handleRemove"
          :on-success="(($event)=>{handleAvatarSuccess($event, 'partyVoluntaryFile')})"
          :before-upload="beforeAvatarUpload"
          :before-remove="beforeRemove"
          :file-list="fileList"
          :multiple="false"
    ...
    methods:{
      ...
      onSubmit(formName){
        let url = this.pageType == 'org' ? 
                `${this.$util.basePath}/hr_join_party_application/develop/register/from_org/${this.orgCode}`:
                `${this.$util.basePath}/hr_join_party_application/develop/register`;
