            ...
            <el-tabs type="card" v-model="activeTab">
              ...
              <el-tab-pane label="组织架构" name="construction">
                ...
                <construction 
                  ...
                  :treeId="partyId || 0" 
                  @treeChanged="initTree"
                  @treeDeleted="initTree">
                </construction>
              </el-tab-pane>
              <el-tab-pane label="党员花名册" name="roster" v-if="partyId">
              ...
              <el-tab-pane label="权限管理" name="authority" v-if="partyId">
                <authority v-if="activeTab == 'authority'" :treeId="partyId"></authority>
              </el-tab-pane>
              ...
              <el-tab-pane label="统计图" name="charts" v-if="partyId">
                <charts v-if="activeTab == 'charts'" :treeId="partyId"></charts>
              </el-tab-pane>
              <el-tab-pane label="表单调整" name="fromEdit" v-if="partyId">
                <fromedit v-if="activeTab == 'fromEdit'" :treeId="partyId"></fromedit>
              </el-tab-pane>
            </el-tabs>
          </el-col>
        </el-row>
...
<script>
  import qs           from "qs"
  import intro        from "./partyManageComponents/Intro.vue"
  import construction from "./partyManageComponents/Construction.vue"
  import roster       from "./partyManageComponents/Roster.vue"
  import leader       from "./partyManageComponents/Leader.vue"
  import authority    from "./partyManageComponents/Authority.vue"
  import information  from "../common/people_import/Information.vue"
  import charts       from "./partyManageComponents/Charts.vue"
  import fromEdit       from "./partyManageComponents/FromEdit.vue"
  export default {
    components: {
      "intro":        intro,
    ...
    methods: {
      ...
      initTree () {
        let self = this
        self.getDataList(`/party_orgs/${JSON.parse(sessionStorage.getItem("orgId"))}/partyorgtree`, "partyListData", false).then((response) => {
          if(response) {
      ...
      getDataList (url, target, multi) {
        ...
        let p = new Promise(function (resolve, reject) {
          self.$request.requestAxios(config.method, config.url, null, (response) => {
